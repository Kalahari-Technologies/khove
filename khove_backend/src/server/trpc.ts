...
/**
 * Build the tRPC context from the Express request. Clerk auth is read off
 * `req` (populated by `clerkMiddleware()` from `@clerk/express`); the active
 * workspace comes from the `x-workspace-id` header the frontend link injects.
 */
export async function createTRPCContext(opts: {
  ...
  const workspaceId = opts.req.header("x-workspace-id");
  if (workspaceId) {
