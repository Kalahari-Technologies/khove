...
function ToolChip({
  ...
  return (
    <motion.div
      variants={item}
      className="inline-flex items-center gap-1.5 rounded-full border border-ink/[0.08] bg-ink/[0.03] px-2.5 py-1 text-[11px] text-ink/60"
    >
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-24 sm:py-32">
      ...
      <motion.div
        ...
        variants={container}
        className="relative mx-auto mt-14 max-w-2xl overflow-hidden rounded-2xl border border-ink/[0.08] bg-ink/[0.02]"
      >
        {/* accent wash */}
        <div
        ...
        {/* Window chrome */}
        <div className="relative flex items-center gap-2 border-b border-ink/[0.06] px-4 py-3">
          <div className="flex gap-1.5">
            <span className="h-2.5 w-2.5 rounded-full bg-ink/[0.12]" />
            <span className="h-2.5 w-2.5 rounded-full bg-ink/[0.12]" />
            <span className="h-2.5 w-2.5 rounded-full bg-ink/[0.12]" />
          </div>
        ...
        <div className="relative flex flex-col gap-4 p-5 sm:p-7">
          ...
          <motion.div variants={item} className="flex justify-end">
            <div className="max-w-[85%] rounded-2xl rounded-br-md bg-ink/[0.06] px-4 py-2.5 text-[13px] text-ink/80">
          ...
          <motion.div variants={item} className="flex justify-start">
            <div className="max-w-[92%] rounded-2xl rounded-bl-md border border-ink/[0.06] bg-ink/[0.02] px-4 py-3.5">
              <p className="mb-3 text-[13px] text-ink/55">
                Here&apos;s your morning, in priority order:
              </p>
              <div className="flex flex-col gap-2.5">
                <AnswerLine color={ACCENTS.planner}>
                  <span className="text-ink/85">PR #471</span> on{" "}
                  <span className="text-ink/85">web</span> has failing checks —
                  it blocks the 2:00 PM release.
                </AnswerLine>
                <AnswerLine color={ACCENTS.chat}>
                  Task <span className="text-ink/85">“API sync”</span> is due
          ...
          <motion.div variants={item} className="flex flex-wrap gap-2 pl-1">
            <button className="group inline-flex items-center gap-1.5 rounded-lg bg-ink px-3.5 py-2 text-[12px] font-medium text-paper transition-colors hover:bg-ink/90">
              Yes, do both
              <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
            </button>
            <button className="rounded-lg border border-ink/[0.10] bg-ink/[0.03] px-3.5 py-2 text-[12px] font-medium text-ink/70 transition-colors hover:bg-ink/[0.06]">
              Just the issue
            </button>
          </motion.div>
        </div>
