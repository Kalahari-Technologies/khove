  ...
  return (
    <section id="pricing" className="mx-auto max-w-6xl px-6 py-24 sm:py-32">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <p className="mt-4 text-[16px] text-ink/50">
          Every integration is free on every plan — you only pay for AI actions.
        </p>

        {/* Interval toggle */}
        <div className="mt-8 inline-flex items-center gap-1 rounded-lg border border-ink/[0.08] bg-ink/[0.03] p-1">
          {[
            ...
            <button
              ...
              className={cn(
                ...
                annual === opt.key ? "text-paper" : "text-ink/60 hover:text-ink"
              )}
            >
              {annual === opt.key && (
                <motion.span
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isPopular
                  ? "border-ink/[0.2] bg-ink/[0.04]"
                  : "border-ink/[0.08] bg-ink/[0.02]"
              )}
            >
              {isPopular && (
                <span className="absolute -top-2.5 left-6 rounded-full bg-ink px-2.5 py-0.5 text-[11px] font-semibold text-paper">
                  Most popular
                </span>
              )}

              <h3 className="text-[15px] font-semibold text-ink">{p.name}</h3>
              <p className="mt-1 text-[13px] text-ink/45">{p.description}</p>

              <div className="mt-5 flex items-baseline gap-1.5">
                <span className="text-[34px] font-semibold tracking-tight text-ink">
                  {price.amount}
                </span>
                <span className="text-[13px] text-ink/40">{price.suffix}</span>
              </div>
              <div className="mt-1 h-4 text-[12px] text-ink/35">
              ...
              <ul className="mt-6 flex flex-col gap-2.5">
                {bullets(tier).map((b) => (
                  <li key={b} className="flex items-start gap-2.5 text-[13px] text-ink/60">
      ...
      {/* Enterprise strip */}
      <div className="mt-4 flex flex-col items-center justify-between gap-4 rounded-2xl border border-ink/[0.08] bg-ink/[0.02] px-6 py-5 sm:flex-row">
        <div>
          <h3 className="text-[15px] font-semibold text-ink">Enterprise</h3>
